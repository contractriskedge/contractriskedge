...
prompt_registry.register(PromptTemplate(
    ...
    version=1,
    template="""
You are a senior contract negotiation specialist. Review ALL of the following risk findings
...
""",
    system_prompt=(
        "You are a senior contract negotiation specialist. "
        "Analyze ALL findings and return redlines for each in a single JSON response. "
        "NEVER include section numbers in proposed_text. "
        "Always respond in valid JSON."
    ),
    response_schema={"type": "json_object"},
))
