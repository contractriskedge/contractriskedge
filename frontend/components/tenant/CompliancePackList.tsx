/**
 * CompliancePackList — Manage compliance packs (list, create, delete).
 */
...
export function CompliancePackList() {
  const { data: packs, isLoading, error, refetch } = useCompliancePacks();
  const createPack = useCreateCompliancePack();
  const [showForm, setShowForm] = useState(false);
  const [name, setName] = useState("");
  const [region, setRegion] = useState("us_federal");
    ...
    createPack.mutate(
      {
        ...
        forbidden_clause_categories: [],
        jurisdiction_rules: [],
        is_active: true,
      },
      {
        onSuccess: () => {
          showFeedback("success", `Compliance pack "${name}" created`);
  ...
  const handleDelete = (_packId: string, _packName: string) => {
    // Delete not supported by backend API
    showFeedback("error", "Delete not available for compliance packs");
  };
  ...
  if (error) return (
    <div className="text-center py-12">
      ...
      <p className="text-sm text-gray-500 mb-2">Failed to load compliance packs</p>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-gray-900">Compliance Packs ({packs?.length ?? 0})</h3>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select value={region} onChange={(e) => setRegion(e.target.value)}
                className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-400"
              >{REGIONS.map((r) => <option key={r.value} value={r.value}>{r.label}</option>)}</select>
            </div>
          </div>
      ...
      {(!packs || packs.length === 0) && !showForm ? (
        <div className="text-center py-12 border-2 border-dashed border-gray-200 rounded-lg">
          ...
          <p className="text-sm text-gray-500">No compliance packs yet</p>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-sky-50 text-sky-700">
                    {REGIONS.find((r) => r.value === pack.region)?.label ?? pack.region}
                  </span>
                ...
                <p className="text-xs text-gray-400 mt-1">
                  {pack.regulations?.length} regulation(s) · {pack.required_clause_categories?.length} required · {pack.forbidden_clause_categories?.length} forbidden · v{pack.version}
                </p>
              </div>
              <button onClick={() => handleDelete(pack.pack_id, pack.name)}
                className="p-1.5 text-gray-400 hover:text-red-500 transition-colors" title="Delete pack"
