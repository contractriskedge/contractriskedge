...
import {
  ...
  GitBranch, Target, TrendingUp, TrendingDown, Minus,
} from "lucide-react";
import type { NegotiationKpi } from "./types";
...
export function NegotiationKpiCards({ metrics, onKpiClick }: { metrics: NegotiationKpi[]; onKpiClick?: (id: string) => void }) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2.5">
      {metrics.map((m, i) => {
        const TrendIcon = m.trendDirection === "up" ? TrendingUp : m.trendDirection === "down" ? TrendingDown : Minus;
        const isBadUp = (m.id === "high-risk-clauses" || m.id === "escalated-issues" || m.id === "open-redlines");
        const tc = (m.trendDirection === "up" && isBadUp) || (m.trendDirection === "down" && !isBadUp && m.id !== "avg-cycle-time") ? "text-red-500" : m.trendDirection === "neutral" ? "text-gray-400" : "text-green-500";
        ...
        return (
          ...
          >
            ...
            <div className="flex items-center gap-1 mt-1 pt-1 border-t border-gray-50">
              ...
              <span className={`text-[10px] font-semibold tabular-nums ${tc}`}>
                {m.trend > 0 ? "+" : ""}{m.trend}%
              </span>
            </div>
          </motion.button>
