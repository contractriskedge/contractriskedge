...
function InsightCard({ insight, index }: { insight: ExecutiveInsight; index: number }) {
        ...
        <div className="flex-1 min-w-0">
          ...
          <AnimatePresence>{expanded && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="mt-2 space-y-2 overflow-hidden">
              <div className="flex items-center gap-1.5 text-[10px] text-navy-700 bg-navy-50 px-2 py-1 rounded"><TrendingUp className="w-3 h-3" /><span className="font-medium">Business Impact:</span> {insight.businessImpact}</div>
              <div className="p-2 bg-navy-50 rounded border border-navy-100">
                <p className="text-[10px] font-semibold text-navy-700 uppercase tracking-wider mb-0.5">Recommended Action</p>
...
export function ExecutiveAiInsights({ insights }: { insights: ExecutiveInsight[] }) {
  const critical = insights.filter((i) => i.severity === "critical").length;
  const warning = insights.filter((i) => i.severity === "warning").length;
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2"><Brain className="w-4.5 h-4.5 text-navy-700" /><h2 className="text-sm font-semibold text-navy-900">AI Executive Intelligence</h2></div>
