...
import { FileText, TrendingUp, BarChart3, CheckCircle, RefreshCw, Globe, Brain, Archive, TrendingUp as TUp, TrendingDown, Minus } from "lucide-react";
import type { ClauseKpi } from "./types";

const iconMap: Record<string, React.ReactNode> = {
  FileText: <FileText className="w-4 h-4" />, TrendingUp: <TrendingUp className="w-4 h-4" />,
  BarChart3: <BarChart3 className="w-4 h-4" />, CheckCircle: <CheckCircle className="w-4 h-4" />,
  RefreshCw: <RefreshCw className="w-4 h-4" />, Globe: <Globe className="w-4 h-4" />,
  Brain: <Brain className="w-4 h-4" />, Archive: <Archive className="w-4 h-4" />,
};
...
export function ClauseKpiCards({ metrics }: { metrics: ClauseKpi[] }) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2.5">
      {metrics.map((m, i) => {
        const TrendIcon = m.trendDirection === "up" ? TUp : m.trendDirection === "down" ? TrendingDown : Minus;
        const isBad = m.trendDirection === "up" && (m.id === "deviations" || m.id === "ai-improvements" || m.id === "deprecated");
