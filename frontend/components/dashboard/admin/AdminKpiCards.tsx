...
import { Users, Shield, Brain, Lock, Link, Activity, FileSearch, ShieldCheck, TrendingUp, TrendingDown, Minus } from "lucide-react";
import type { AdminKpi } from "./types";

const iconMap: Record<string, React.ReactNode> = {
  Users: <Users className="w-4 h-4" />, Shield: <Shield className="w-4 h-4" />, Brain: <Brain className="w-4 h-4" />,
  Lock: <Lock className="w-4 h-4" />, Link: <Link className="w-4 h-4" />, Activity: <Activity className="w-4 h-4" />,
...
export function AdminKpiCards({ metrics }: { metrics: AdminKpi[] }) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2.5">
      {metrics.map((m, i) => {
        const TrendIcon = m.trendDirection === "up" ? TrendingUp : m.trendDirection === "down" ? TrendingDown : Minus;
        const isBad = (m.trendDirection === "up" && (m.id === "ai-violations" || m.id === "audit-events" || m.id === "failed-logins")) || (m.trendDirection === "down" && (m.id === "security-alerts" || m.id === "system-health"));
        const tc = isBad ? "text-red-500" : "text-green-500";
        const strokeColor = m.severity === "critical" ? "#EF4444" : m.severity === "high" ? "#F97316" : m.severity === "medium" ? "#EAB308" : m.severity === "low" ? "#22C55E" : "#3B82F6";
        return (
