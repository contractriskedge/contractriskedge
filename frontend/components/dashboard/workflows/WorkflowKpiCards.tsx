...
import { Clock, AlertTriangle, AlertOctagon, ArrowUpCircle, CheckCircle, Zap, Ban, Bot, TrendingUp, TrendingDown, Minus } from "lucide-react";
import type { WorkflowKpi } from "./types";

const iconMap: Record<string, React.ReactNode> = {
  Clock: <Clock className="w-4 h-4" />, AlertTriangle: <AlertTriangle className="w-4 h-4" />,
  AlertOctagon: <AlertOctagon className="w-4 h-4" />, ArrowUpCircle: <ArrowUpCircle className="w-4 h-4" />,
  CheckCircle: <CheckCircle className="w-4 h-4" />, Zap: <Zap className="w-4 h-4" />,
  Ban: <Ban className="w-4 h-4" />, Bot: <Bot className="w-4 h-4" />,
};
...
export function WorkflowKpiCards({ metrics }: { metrics: WorkflowKpi[] }) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2.5">
      {metrics.map((m, i) => {
        const TrendIcon = m.trendDirection === "up" ? TrendingUp : m.trendDirection === "down" ? TrendingDown : Minus;
        const isBad = (m.trendDirection === "up" && (m.id === "pending-approvals" || m.id === "escalated" || m.id === "blocked")) || (m.trendDirection === "down" && (m.id === "completion-rate" || m.id === "automation-rate"));
        ...
        return (
          <motion.div key={m.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
            ...
            <div className="flex items-center justify-between mb-2">
              <div className={`w-7 h-7 rounded-lg flex items-center justify-center bg-gradient-to-br ${m.color} text-white shadow-xs`}>{iconMap[m.icon] || <Clock className="w-4 h-4" />}</div>
