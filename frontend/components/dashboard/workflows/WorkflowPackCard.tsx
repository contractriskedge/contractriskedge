...
export function WorkflowPackCard({ pack, viewMode, onClick, onArchive, onClone }: Props) {
  ...
  const ActionMenu = () => (
    ...
    >
      ...
      <div
        onClick={(e) => { e.stopPropagation(); setMenuOpen(false); onArchive?.(pack.pack_id); }}
        className="flex items-center gap-2 px-3 py-2 text-xs text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 cursor-pointer"
      >
