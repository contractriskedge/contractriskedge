...
import { Brain, AlertTriangle, AlertOctagon, Clock, Lightbulb, ChevronRight, Sparkles, Workflow } from "lucide-react";
...
function InsightCard({ insight, index }: { insight: WorkflowInsight; index: number }) {
        ...
        <div className="flex-1 min-w-0">
          ...
          <AnimatePresence>{expanded && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="mt-2 space-y-2 overflow-hidden">
              <div className="flex gap-1 flex-wrap">{insight.impactedWorkflows.slice(0, 5).map((w) => (
                <span key={w} className="text-[9px] px-1.5 py-0.5 rounded bg-navy-50 text-navy-700 font-mono">{w}</span>
              ))}</div>
...
export function WorkflowAiInsights({ insights }: { insights: WorkflowInsight[] }) {
  const critical = insights.filter((i) => i.severity === "critical").length;
  const warning = insights.filter((i) => i.severity === "warning").length;
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2"><WorkflowIcon /><h2 className="text-sm font-semibold text-navy-900">AI Workflow Intelligence</h2></div>
...
function WorkflowIcon() {
  return (
    <svg className="w-4.5 h-4.5 text-navy-700" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
