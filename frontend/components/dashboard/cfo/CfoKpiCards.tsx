...
import {
  ...
  TrendingDown, Clock, PiggyBank, TrendingUp, TrendingDown as Td, Minus,
} from "lucide-react";
import type { CfoKpi } from "./types";
...
export function CfoKpiCards({ metrics, onKpiClick }: { metrics: CfoKpi[]; onKpiClick?: (id: string) => void }) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2.5">
      {metrics.map((m, i) => {
        const TrendIcon = m.trendDirection === "up" ? TrendingUp : m.trendDirection === "down" ? Td : Minus;
        const isBadUp = m.id === "exposure-risk" || m.id === "uncapped-liability" || m.id === "renewal-commitments" || m.id === "vendor-concentration" || m.id === "revenue-leakage";
