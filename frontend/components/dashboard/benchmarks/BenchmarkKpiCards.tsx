...
import { Database, AlertTriangle, TrendingUp, Handshake, PieChart, DollarSign, Swords, Shield, TrendingUp as TUp, TrendingDown, Minus } from "lucide-react";
import type { BenchmarkKpi } from "./types";

const iconMap: Record<string, React.ReactNode> = {
  Database: <Database className="w-4 h-4" />, AlertTriangle: <AlertTriangle className="w-4 h-4" />,
  TrendingUp: <TrendingUp className="w-4 h-4" />, Handshake: <Handshake className="w-4 h-4" />,
  PieChart: <PieChart className="w-4 h-4" />, DollarSign: <DollarSign className="w-4 h-4" />,
  Swords: <Swords className="w-4 h-4" />, Shield: <Shield className="w-4 h-4" />,
};
...
export function BenchmarkKpiCards({ metrics }: { metrics: BenchmarkKpi[] }) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2.5">
      {metrics.map((m, i) => {
        const TrendIcon = m.trendDirection === "up" ? TUp : m.trendDirection === "down" ? TrendingDown : Minus;
        const isBad = (m.trendDirection === "up" && (m.id === "deviations" || m.id === "above-market" || m.id === "pricing-variance" || m.id === "aggressiveness")) || (m.trendDirection === "down" && m.id === "negotiation-opps");
        ...
        return (
          <motion.div key={m.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
            ...
            <div className="flex items-center justify-between mb-2">
              <div className={`w-7 h-7 rounded-lg flex items-center justify-center bg-gradient-to-br ${m.color} text-white shadow-xs`}>{iconMap[m.icon] || <Database className="w-4 h-4" />}</div>
