...
import { Brain, AlertTriangle, AlertOctagon, Clock, Lightbulb, ChevronRight, Sparkles, Share2 } from "lucide-react";
        ...
        <div className="flex-1 min-w-0">
          ...
          <AnimatePresence>{expanded && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="mt-2 space-y-2 overflow-hidden">
              <div className="flex gap-1 flex-wrap">{insight.impactedContracts.slice(0, 5).map((c) => (
                <span key={c} className="text-[9px] px-1.5 py-0.5 rounded bg-navy-50 text-navy-700 font-mono">{c}</span>
              ))}{insight.impactedContracts.length > 5 && <span className="text-[9px] text-gray-400">+{insight.impactedContracts.length - 5}</span>}</div>
...
export function RelationshipAiInsights({ insights }: { insights: RelationshipInsight[] }) {
  const critical = insights.filter((i) => i.severity === "critical").length;
  const warning = insights.filter((i) => i.severity === "warning").length;
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2"><Share2 className="w-4.5 h-4.5 text-navy-700" /><h2 className="text-sm font-semibold text-navy-900">Relationship Intelligence</h2></div>
      ...
      <div className="flex items-center gap-3 px-3 py-2 bg-gradient-to-r from-navy-50 to-indigo-50 rounded-lg border border-navy-100">
        ...
        <span className="text-[11px] font-medium text-navy-700">
          <span className="font-bold text-red-600">{critical} critical</span> and <span className="font-bold text-orange-600">{warning} warnings</span> detected
        </span>
      </div>
