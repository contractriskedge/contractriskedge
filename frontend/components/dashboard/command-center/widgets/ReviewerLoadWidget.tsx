...
export function ReviewerLoadWidget({ reviewerData }: ReviewerLoadWidgetProps) {
  ...
  // Only show reviewers with active reviews for the workload table
  const activeReviewers = reviewerData.reviewer_details.filter((r) => r.active_reviews > 0);
  // Show historical-only reviewers (completed reviews but no active) in a separate section
  const historicalReviewers = reviewerData.reviewer_details.filter((r) => r.active_reviews === 0 && r.completed_reviews > 0);
  const overloadedReviewers = activeReviewers.filter((r) => r.is_overloaded);
    ...
    <div className="space-y-3">
      ...
      {overloadedReviewers.length > 0 && (
        <div className="px-3 py-2 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800">
          <p className="text-xs font-medium text-red-700 dark:text-red-300">
            ...
            ({overloadedReviewers.map((r) => r.reviewer_name).join(", ")})
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="p-2 rounded-lg bg-gray-50 dark:bg-navy-700 text-center">
          <span className="text-lg font-bold text-navy-900 dark:text-white">{reviewerData.active_reviewers}</span>
      ...
      {activeReviewers.length > 0 ? (
        <div>
          ...
          <div className="space-y-1">
            {activeReviewers.map((reviewer) => {
              const active = reviewer.active_reviews;
              const totalWork = active + reviewer.completed_reviews;
              const capacity = 20;
              const utilization = Math.min(totalWork / capacity, 1);
              return (
                <div key={reviewer.reviewer_id} className="flex items-center gap-2 group">
                  <span className="text-xs text-gray-600 dark:text-gray-400 w-24 truncate" title={`${reviewer.reviewer_name} — ${active} active, ${reviewer.completed_reviews} completed`}>
        ...
        <div className="text-center py-4 text-xs text-gray-400">
          <p>No active reviewers with current workload</p>
          {historicalReviewers.length > 0 && (
            <p className="text-[10px] text-gray-400 mt-1">
              {historicalReviewers.length} reviewer{historicalReviewers.length > 1 ? "s" : ""} have completed reviews but no active assignments
            </p>
          )}
        </div>
      )}
